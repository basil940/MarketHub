...
// Orders
async function placeOrder(shipping_address, redeem_points = 0) {
    const res = await fetch('/api/orders/place/', {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ shipping_address, redeem_points })
    });
    return res.json();
}
...
// Payment
async function createCheckoutSession(order_id) {
    const res = await fetch(`/api/orders/${order_id}/checkout/`, {
        method: 'POST',
        headers: authHeaders()
    });
    return res.json();
...
async function submitReview(slug, rating, comment) {
    const res = await fetch(`${BASE}/products/${slug}/reviews/`, {
        method: 'POST',
        headers: authHeaders(),
